...
resource "azurerm_subnet" "vm_windows_subnet" {
  ...
  address_prefixes     = [cidrsubnet(azurerm_virtual_network.pyp.address_space.0, 8, 1)]
}
...
resource "azurerm_subnet" "vm_linux_subnet" {
  ...
  resource_group_name  = azurerm_resource_group.pyp.name
  virtual_network_name = azurerm_virtual_network.pyp.name
  address_prefixes     = [cidrsubnet(azurerm_virtual_network.pyp.address_space.0, 8, 2)]
}

resource "azurerm_public_ip" "vm_windows" {
  count               = var.num_windows_hosts
  name                = "${var.env_name}-win${count.index + 1}-publicip"
  location            = azurerm_resource_group.pyp.location
  resource_group_name = azurerm_resource_group.pyp.name
  allocation_method   = "Dynamic"
  domain_name_label   = "${var.env_name}-win${count.index + 1}"
...
resource "azurerm_public_ip" "vm_linux" {
  ...
  name                = "${var.env_name}-linux${count.index + 1}-publicip"
  location            = azurerm_resource_group.pyp.location
  resource_group_name = azurerm_resource_group.pyp.name
  allocation_method   = "Dynamic"
  domain_name_label   = "${var.env_name}-linux${count.index + 1}"
